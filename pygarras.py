...
window_dimensions = (1920, 1080)  # in pixels
...
def manage_inputs(input_type):
    if input_type == "key":
        ...
        if event.key == pygame.K_f:
            ...
            new_entity.guns.append(Gun(15, 10, -1.4, 0, 0, 0, COL_GREY))
            new_entity.guns.append(Gun(15, 10, -1.4, 0, 0, 120, COL_GREY))
            new_entity.guns.append(Gun(15, 10, -1.4, 0, 0, 240, COL_GREY))
...
def draw_guns(entity):
    for gun in entity.guns:
        a = entity.angle + gun.angle
        length = gun.length * 2
        width = gun.width
        darker = (gun.col[0] // 2, gun.col[1] // 2, gun.col[2] // 2)

        newAspect = gun.aspect if gun.aspect > 1 else gun.aspect if 0<gun.aspect<=1 else 1
        newAspect2 = abs(gun.aspect) if gun.aspect < -1 else abs(gun.aspect) if -1<gun.aspect<=0 else 1

        offset_x = gun.x * math.cos(a) - gun.y * math.sin(a)
        offset_y = gun.x * math.sin(a) + gun.y * math.cos(a)
        base_x = entity.x + offset_x * 2
        base_y = entity.y + offset_y * 2

        p1x = base_x + width * math.sin(a) * newAspect2
        p1y = base_y - width * math.cos(a) * newAspect2

        p2x = base_x - width * math.sin(a) * newAspect2
        p2y = base_y + width * math.cos(a) * newAspect2

        p3x = base_x + length * math.cos(a) - width * math.sin(a) * newAspect
        p3y = base_y + length * math.sin(a) + width * math.cos(a) * newAspect

        p4x = base_x + length * math.cos(a) + width * math.sin(a) * newAspect
        p4y = base_y + length * math.sin(a) - width * math.cos(a) * newAspect
...
def draw_hp_bar(entity):
    ...
    if entity.alive  and entity.health < entity.max_health:
        ...
        pygame.draw.rect(window, COL_GREEN, (entity.x - entity.size, entity.y + entity.size + 10, entity.size * 2 * ((entity.health / entity.max_health)), bar_width),0,10)


player = Entity(initial_position[0], initial_position[1], 20, 0, COL_BLUE, 100)
player.guns.append(Gun(18, 10, -1.4, 0, 0, 0, COL_GREY))
